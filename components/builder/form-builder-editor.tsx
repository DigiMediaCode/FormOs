...
function createField(type: FormFieldType, order: number): FormBuilderField {
  return {
    ...
    required: false,
    order,
    options: fieldSupportsOptions(type) ? ["Option 1", "Option 2"] : [],
    content: "",
...
export function FormBuilderEditor({
  ...
  function cleanFieldOptions(fieldId: string) {
    setFields((currentFields) =>
      normalizeOrders(
        currentFields.map((field) =>
          field.id === fieldId
                      ...
                      {field.options.map((option, optionIndex) => (
                        <div className="grid gap-2 sm:grid-cols-[1fr_auto]" key={`${field.id}-${optionIndex}`}>
                          <input
                            ...
                            onBlur={() => cleanFieldOptions(field.id)}
                            onChange={(event) =>
              ...
              return (
                <label className="flex flex-col gap-1.5 text-sm font-normal text-slate-700" key={field.id}>
                  ...
                  {field.type === "textarea" || field.type === "address" ? (
                    ...
                    field.options.length > 0 ? (
                      <span className="grid gap-2 rounded-md border border-slate-300 bg-slate-50 px-3 py-2 text-sm text-slate-700">
                        {field.options.map((option) => (
