...
export type TemplateGalleryCard = {
  slug: string;
  title: string;
  category: string;
  description: string;
    ...
    <div>
      <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
        ...
        <div className="relative lg:w-72">
          ...
          <input
            className="w-full rounded-full border border-slate-200 bg-white py-2 pl-9 pr-3 text-sm text-slate-800 outline-none transition focus:border-blue-500 focus:ring-2 focus:ring-blue-100"
      ...
      {filtered.length > 0 ? (
        <div className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {filtered.map((template) => (
            ...
            >
              ...
              <p className="mt-2 line-clamp-3 flex-1 text-xs leading-5 text-slate-600">
              ...
              <div className="mt-4">
                {template.canCreate ? (
                  ...
                  <Link
                    className="block w-full rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-center text-sm font-semibold text-slate-700 transition hover:border-blue-200 hover:bg-blue-50"
                    href="/dashboard/settings/billing"
                  >
                    {template.minimumPlanName
                      ? `Requires ${template.minimumPlanName}`
                      : template.ctaLabel}
                  </Link>
                )}
              </div>
        ...
        <p className="mt-8 rounded-2xl border border-slate-200 bg-white p-6 text-center text-sm text-slate-600">
